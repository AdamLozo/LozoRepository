# Date Table Function - Fiscal Year Ending

Generates a comprehensive date table where **fiscal year equals the ending calendar year**.

## Fiscal Year Logic

When `FYStartMonth = 7` (July):
- **FY 2024** = July 1, 2023 through June 30, 2024
- The fiscal year name matches the **ending** calendar year
...
|-----------|------|----------|-------------|
| StartDate | date | Yes | First date in the table |
| EndDate | date | Yes | Last date in the table |
| FYStartMonthNum | number | No | Month number fiscal year starts (1-12). Default: 1 (January) |
| Holidays | list | No | List of holiday dates for business day calculations |
| WDStartNum | number | No | Weekday numbering: 0 (default, Mon=0) or 1 (Mon=1) |
| AddRelativeNetWorkdays | logical | No | Add relative network days column. Default: false |

## Key Formula Logic

The fiscal year calculation that makes this "year ending":

```powerquery
FY number = if [Month] >= FYStartMonth then [Year] + 1 else [Year]
```

This adds 1 to the calendar year when past the fiscal year start month.

## Generated Columns

### Calendar Columns
- Date, Year, Quarter, Month, Week, Day columns
- ISO 8601 week numbering
- Day of week, day of year, day of month
- Month names (full, short, initial)

### Fiscal Columns
- Fiscal Year (e.g., "FY2024")
- Fiscal Quarter (e.g., "FQ1 2024")
- Fiscal Period (e.g., "FP01 2024")
- Fiscal Week (e.g., "FW01 2024")
- Fiscal Period Number (1-12)

### Flags & Offsets
- Current period flags (IsCurrentFY, IsCurrentFQ, IsCurrentFP, IsCurrentFW)
- Offset columns (CurrYearOffset, CurrQuarterOffset, etc.)
- Completed flags (YearCompleted, QuarterCompleted, etc.)
- Prior year to date flags (IsPYTD, IsPFYTD)

### Business Day Columns (when Holidays provided)
- IsWeekDay
- IsHoliday
- IsBusinessDay
...
```powerquery
// Create date table with July 1 fiscal year start (year ending)
DateTable-FiscalYearEnd(
    #date(2020, 7, 1),
    #date(2026, 6, 30),
    7,                    // Fiscal year starts in July
    null,                 // No holidays
    0,                    // Standard weekday numbering
    false                 // No relative network days
)
```

## Result for July 2023

| Date | Year | Month | FY number | Fiscal Year |
|------|------|-------|-----------|-------------|
| 2023-07-15 | 2023 | 7 | **2024** | FY2024 |

Notice: July 2023 belongs to **FY2024** (the ending year).

## Use Case

This function is ideal when:
- Your fiscal year is named for when it **ends**
- Example: FY2024 ends on June 30, 2024
- Common in government and public sector organizations

## Credits

**Author:** Melissa de Korte  
**Version:** 2.04 - Optimized Version  
**Source:** [Enterprise DNA Forum](https://forum.enterprisedna.co/t/extended-date-table-power-query-m-function/6390)
